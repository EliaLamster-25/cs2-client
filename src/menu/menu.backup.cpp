...
void Menu::drawAimbotPanel() {
    ...
    if (m_aimSubTab == 0) {
        ...
        m_gui.dummy(8.f);
        m_gui.label("HUMANIZATION", Theme::TEXT_MUTED, 15.f);
        m_gui.dummy(8.f);
        m_gui.toggleCheckbox("aimHumEn", "Humanize aim", &g_cfg.aimHumanizeEnabled, 44.f);
        static const char* kAssistStyles[] = { "Direct (classic smooth)", "Adaptive (path-aware)" };
        int assistStyle = std::clamp(g_cfg.aimAssistStyle, 0, 1);
        m_gui.label("Assist style", Theme::TEXT_MUTED, 14.f);
        if (m_gui.comboBox("aimAssistStyle", kAssistStyles, 2, &assistStyle))
            g_cfg.aimAssistStyle = assistStyle;
        if (assistStyle == 0) {
            m_gui.label("Linear smoothing only — no reaction delay or path correction.", Theme::TEXT_MUTED, 13.f);
        } else {
            m_gui.label("Adjusts to your flick path, brakes near target, corrects overshoot.", Theme::TEXT_MUTED, 13.f);
        }
        m_gui.dummy(6.f);
        static const char* kHumModes[] = { "Auto (calibrated)", "Manual" };
        int humMode = std::clamp(g_cfg.aimHumanizeMode, 0, 1);
        m_gui.label("Humanization mode", Theme::TEXT_MUTED, 14.f);
        if (m_gui.comboBox("aimHumMode", kHumModes, 2, &humMode)) {
            g_cfg.aimHumanizeMode = humMode;
            g_cfg.aimHumanizeUseProfile = (humMode == 0);
        }
        if (humMode == 0 && assistStyle == 1) {
            m_gui.sliderFloatValue("aimCalibStr", "Assist intensity",
                &g_cfg.aimCalibAssistStrength, 0.f, 1.f, "%.2f", 74.f);
            m_gui.label("Higher = stronger adaptive support. Lower = lighter touch.", Theme::TEXT_MUTED, 13.f);
        } else if (humMode == 0 && assistStyle == 0) {
            m_gui.label("Direct mode uses calibrated smoothing per weapon (no adaptive layer).", Theme::TEXT_MUTED, 13.f);
        }
        if (humMode == 0) {
            if (g_cfg.calibrationFrameworkComplete) {
                m_gui.label("Auto mode uses per-weapon profiles from calibration.", Theme::TEXT_MUTED, 13.f);
            } else if (g_cfg.aimStyleProfileValid) {
                char prof[320];
                std::snprintf(prof, sizeof(prof),
                    "Assist: RT %.0fms  smooth %.1f  speed %.0f deg/s  (you: %.0fms / %.1f / %.0f)",
                    g_cfg.aimStyleReactionMs, g_cfg.aimStyleSmooth, g_cfg.aimStyleFlickSpeed,
                    g_cfg.aimStyleMeasuredReactionMs, g_cfg.aimStyleMeasuredSmooth,
                    g_cfg.aimStyleMeasuredFlickSpeed);
                m_gui.label(prof, Theme::TEXT_MUTED, 13.f);
            } else {
                m_gui.label("Run calibration to tune assist to your aim (not mimic it).", Theme::TEXT_MUTED, 13.f);
            }
        } else {
            m_gui.sliderFloatValue("aimHumStr", "Humanize strength", &g_cfg.aimHumanizeStrength, 0.f, 1.f, "%.2f", 74.f);
            ...
            m_gui.sliderFloatValue("aimHumOver", "Overshoot", &g_cfg.aimHumanizeOvershoot, 0.f, 0.35f, "%.2f", 74.f);
            m_gui.sliderFloatValue("aimHumSmoothX", "Extra smooth", &g_cfg.aimHumanizeSmoothExtra, 0.f, 12.f, "%.1f", 74.f);
            m_gui.sliderFloatValue("aimHumSpeed", "Speed scale", &g_cfg.aimHumanizeSpeedScale, 0.35f, 1.5f, "%.2f", 74.f);
            m_gui.toggleCheckbox("aimHumPause", "Micro pauses", &g_cfg.aimHumanizeMicroPause, 44.f);
        }
        m_gui.dummy(8.f);
        m_gui.label("AIM CALIBRATION", Theme::TEXT_MUTED, 15.f);
        m_gui.dummy(8.f);
        ...
        if (weaponGroupSupportsRcs(weaponGroup)) {
            ...
            m_gui.label("RCS", Theme::TEXT_MUTED, 14.f);
            m_gui.dummy(6.f);
            m_gui.label("Not used for this weapon class (tap / single-shot).", Theme::TEXT_MUTED, 13.f);
        }
...
void Menu::drawCalibrationPromptOverlay() {
    ...
    r.drawFilledRect(m_winX, m_winY, m_winW, m_winH, 0xAA050508);

    const float modalW = S(440.f);
    const float modalH = S(248.f);
    const float mx = m_winX + (m_winW - modalW) * 0.5f;
    const float my = m_winY + (m_winH - modalH) * 0.36f;
    ...
    r.drawText(f, mx + S(24.f), my + S(22.f), "Aim calibration recommended", Theme::TEXT, 20.f * c);
    r.drawText(f, mx + S(24.f), my + S(58.f),
        "Calibrate once for all weapon classes (pistol, rifle, sniper, SMG, heavy).",
        Theme::TEXT_MUTED, 14.f * c);
    r.drawText(f, mx + S(24.f), my + S(82.f),
        "Flicks tune smoothing & humanization. Spray phase tunes RCS for SMG / rifles / LMG.",
        Theme::TEXT_MUTED, 14.f * c);
    r.drawText(f, mx + S(24.f), my + S(106.f),
        "Use a private server or offline map. Combat features pause during calibration.",
        Theme::TEXT_MUTED, 13.f * c);

    m_gui.setCursor(mx + S(24.f), my + S(148.f));
    m_gui.setItemWidth(modalW - S(48.f));
    if (m_gui.accentButton("calPromptStart", "Start calibration now"))
        AimCalibration::instance().startFullCalibration();
    m_gui.dummy(S(10.f));
    if (m_gui.button("Skip for now", modalW - S(48.f), S(36.f)))
...
void Menu::drawPlayersPanel() {
    ...
    if (isSettings) {
        ...
        m_gui.label("Visibility Backend", Theme::TEXT_MUTED, 14.f);
        ...
        m_gui.separator();
        m_gui.label("ESP Features", Theme::TEXT, 22.f);
        m_gui.dummy(10.f);
        m_gui.toggleCheckbox("plBoxEn", "Box Enabled", &g_cfg.boxEnabled);
        m_gui.toggleCheckbox("plHpEn", "HP Bar Enabled", &g_cfg.hpBarEnabled);
        m_gui.toggleCheckbox("plSkel", "Skeleton Enabled", &g_cfg.skeletonEnabled);
        m_gui.toggleCheckbox("plChams", "Chams Enabled", &g_cfg.chamsEnabled);
        m_gui.toggleCheckbox("plChamsOcc", "Chams Occluded (XQZ)", &g_cfg.chamsOccluded);
    }
...
void Menu::drawExtraPanel() {
    ...
    m_gui.separator();
    m_gui.label("Overlay Control", Theme::TEXT, 22.f);
    m_gui.dummy(10.f);
